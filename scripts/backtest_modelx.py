...
class BacktestEngine:
    ...
    def get_performance_stats(self, risk_per_trade_pct=1.0):
        ...
        # Pips metrics
        total_pips = closed_trades['pnl_pips'].sum()
        avg_win_pips = closed_trades[closed_trades['pnl_pips'] > 0]['pnl_pips'].mean() if winning_trades > 0 else 0
        avg_loss_pips = closed_trades[closed_trades['pnl_pips'] < 0]['pnl_pips'].mean() if losing_trades > 0 else 0
