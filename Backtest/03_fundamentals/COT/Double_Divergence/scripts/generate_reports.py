...
def format_report_with_comparison(stats: dict, htf_timeframe: str, config: dict,
    ...
    lines.append(f"R per Year: {fmt_p2(stats['r_per_year'], p2.get('r_per_year'), 'R')}")
    ...
    lines.append(f"  • Average Winner: +{stats['avg_win_r']:.2f}R")
    lines.append(f"  • Average Loser: {stats['avg_loss_r']:.2f}R")
    ...
    lines.append(f"Long/Short: {stats['long_short_ratio']:.2f}:1 ({stats['long_count']}L / {stats['short_count']}S)")
    ...
    lines.append(f"  • Duration: {stats['max_dd_duration_trades']} trades ({stats['max_dd_duration_days']} days / {stats['max_dd_duration_months']:.1f} months)")
    lines.append(f"  • Recovery: {stats['dd_recovery']}")
    lines.append("")
    ...
    lines.append("=" * 80)
    lines.append(f"R per Month: {stats['r_per_month']:.1f}R avg")
    lines.append(f"R per Year: {stats['r_per_year']:.1f}R avg")
    lines.append("")
    lines.append("Monthly:")
    lines.append(f"  • Best: +{stats['best_month_r']:.1f}R | Worst: {stats['worst_month_r']:.1f}R")
    lines.append(f"  • Profitable: {stats['profitable_months_pct']:.0f}% ({stats['profitable_months']}/{stats['total_months']} months)")
    lines.append("")
    lines.append("Yearly:")
    lines.append(f"  • Best: +{stats['best_year_r']:.1f}R | Worst: {stats['worst_year_r']:.1f}R")
